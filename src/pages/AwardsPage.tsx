...
import { Trophy, Star, Award as AwardIcon, Gift, ChevronDown, ChevronUp, Plane } from 'lucide-react';
            ...
            <div className={`px-4 sm:px-6 py-4 sm:py-5 ${isBlue ? 'bg-amber-50/50' : 'bg-amber-950/20'}`}>
              ...
              <div className={`text-sm leading-relaxed whitespace-pre-line ${
                ...
                {award.mechanism.split('\n').map((line, i) => {
                  const isCondition = line.toLowerCase().includes('điều kiện đủ') || line.toLowerCase().includes('điều kiện cần') || line.toLowerCase().includes('điều kiện ràng buộc');
                  const formattedLine = line.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
                  
                  if (isCondition) {
                    return (
                      <div key={i} className={`mt-3 mb-1 p-3 rounded-xl border shadow-sm flex gap-2 items-start ${
                        isBlue 
                          ? 'bg-amber-50/80 border-amber-200/80 text-amber-900' 
                          : 'bg-amber-500/10 border-amber-500/20 text-amber-200'
                      }`}>
                        <Star className={`shrink-0 w-4 h-4 mt-0.5 ${isBlue ? 'text-amber-600' : 'text-amber-400'}`} fill="currentColor" />
                  ...
                  return (
                    <p key={i} className={line.startsWith('*') || line.startsWith('-') ? 'ml-2 mt-1' : 'mt-1'} dangerouslySetInnerHTML={{ __html: formattedLine }} />
                  );
                })}
              </div>
            </div>
                      ...
                      {/* Render columns generically based on column definition */}
                        {(() => {
                          ...
                          return cells.map((val, ci) => {
                            ...
                            const isMoney = valStr.includes(' đ');
                            const isGEA = valStr.includes('GEA');
                            const isTravel = valStr.toLowerCase().includes('du lịch');
                            const isLabel = ci === 0 && colCount > 3;
                            const isEGC = isLabel && tier.label.includes('EGC');

                            return (
                              <td key={ci} className={`px-2.5 sm:px-4 py-3 sm:py-3.5 text-center whitespace-nowrap align-middle ${
                                isEGC
                                ...
                                {isEGC ? (
                                  ...
                                  <span className="font-black text-red-700 text-base">{valStr}</span>
                                ) : isTravel ? (
                                  <span className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-red-600 via-red-500 to-red-700 text-yellow-100 text-[13px] font-black rounded-xl shadow-[0_4px_12px_rgba(220,38,38,0.4)] border border-red-400/30 whitespace-nowrap">
                                    <Plane size={15} className="text-yellow-300" fill="currentColor" />
                                    {valStr}
                                  </span>
                                ) : isGEA ? (
                                  <span className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white text-xs font-bold rounded-full shadow-sm shadow-amber-500/20">
                                    <Gift size={12} className="text-white/90" />
                                    {valStr}
                                  </span>
                                ) : (
                                  valStr
